...
def glsae(p, M, N_t, rng, sigma=4, gaussian_radius=None, sample_repeat=100, G=16):
...
def gmmae(p, M, N_t, rng, sigma=4, gaussian_radius=None, sample_repeat=100, G=16):
    """Run GMMAE using Gaussian depth sampling and correlation maximization.
    ...
    # Local refinement around the coarse estimate.
    t = np.abs(
        np.mean(samp * np.cos(indices * (theta + t_sharp_list / G - 4) / M * np.pi), -1)
    ).argmax(-1)
    out = (1 - np.cos((theta + t / G - 4) / M * np.pi)) / 2
    return out, queries, depth
